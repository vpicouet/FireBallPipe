...
def ChangeNameGuiderImages(path):
    ...
    for i,file in enumerate(files):
        with fits.open(file) as f:
            ...
            os.rename(name, f.filename()[:-5] + '_pa' +rot +'' + lin +  '_' + date + '.fits')
    return 7971390
...
def revert_names(path):
    ...
    for i,file in enumerate(files):
        with fits.open(file) as f:
            ...
            newname = os.path.dirname(name) + '/'  + os.path.basename(name)[:12] + os.path.basename(name)[-5:]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p','--path',default=os.getcwd(),#type='string',
                        help='''The path of the path to change the guider images''')
    parser.add_argument('-r','--reverse',action='store_true',#type='string',
