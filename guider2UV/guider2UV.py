...
def fit_model(coord, coord_obs, gamma=True, ytilt=False, weight=None):
    ...
    if gamma:
        if not ytilt:
            ...
            row_x = np.hstack(
                (coord_arr * [1.0, -1], np.ones((n, 1)), np.zeros((n, 1)))
            )  # xn - x =  x dgamma - y theta + dx
            row_y = np.hstack(
                (coord_arr[:, ::-1], np.zeros((n, 1)), np.ones((n, 1)))
            )  # yn - y =  y dgamma + x theta      + dy
            ...
            print(
                "gamma: {}\ntheta: {} arcmin\ndx: {} arcsec\ndy: {} arcsec".format(
                    gamma, theta, deltax * 3600, deltay * 3600
                )
            )
            ...
            print(
                "variances: {}\n".format(
                    np.sqrt(np.diag(covar)) / 3600 * [1, 180 / np.pi * 60, 3600, 3600]
                )
            )  #
        else:
            # with gamma, ytilt
            ...
            row_x = np.hstack(
                (
                    coord_arr[:, [0, 1, 1]] * [1.0, -1, -1],
                    np.ones((n, 1)),
                    np.zeros((n, 1)),
                )
            )  # xn - x =  x dgamma - y theta - y tilt + dx
            row_y = np.hstack(
                (coord_arr[:, ::-1], np.zeros((n, 2)), np.ones((n, 1)))
            )  # yn - y =  y dgamma + x theta                              + dy
            mat = np.vstack((row_x, row_y))
            if weight is not None:
                mat = np.diag(np.sqrt(weight)).dot(mat)
            ...
            print(
                "gamma: {}\ntheta: {} arcmin\ntheta_y: {} arcmin\ndx: {} arcsec\ndy: {} arcsec".format(
                    gamma, theta, thetay, deltax * 3600, deltay * 3600
                )
            )
            covar = matinv.dot(matinv.T)
            # accuracy, assuming 1 arcsec measurement error
            print(
                "variances: {}\n".format(
                    ...
                    / 3600
                    * [1, 180 / np.pi * 60, 180 / np.pi * 60, 3600, 3600]
                )
            )  #
        ...
        if not ytilt:
            ...
            row_x = np.hstack(
                (-coord_arr[:, [1]], np.ones((n, 1)), np.zeros((n, 1)))
            )  # xn - x =  - y theta + dx
            row_y = np.hstack(
                (coord_arr[:, [0]], np.zeros((n, 1)), np.ones((n, 1)))
            )  # yn - y =    x theta      + dy
            mat = np.vstack((row_x, row_y))
            if weight is not None:
                mat = np.diag(np.sqrt(weight)).dot(mat)
            ...
            print(
                "variances: {}\n".format(
                    np.sqrt(np.diag(covar)) / 3600 * [180 / np.pi * 60, 3600, 3600]
                )
            )
        else:
            ...
            print("Fitting rotation and translation")
            row_x = np.hstack(
                (-coord_arr[:, [1, 1]], np.ones((n, 1)), np.zeros((n, 1)))
            )  # xn - x =  - y theta - y tilt + dx
            row_y = np.hstack(
                (coord_arr[:, [0]], np.zeros((n, 2)), np.ones((n, 1)))
            )  # yn - y =    x theta                 + dy
            mat = np.vstack((row_x, row_y))
            if weight is not None:
                mat = np.diag(np.sqrt(weight)).dot(mat)
            ...
            print(
                "theta: {} arcmin\ntheta_y: {} arcmin\ndx: {} arcsec\ndy: {} arcsec".format(
                    theta, thetay, deltax * 3600, deltay * 3600
                )
            )
            covar = matinv.dot(matinv.T)
            # accuracy, assuming 1 arcsec measurement error
            print(
                "variances: {}\n".format(
                    np.sqrt(np.diag(covar))
                    / 3600
                    * [180 / np.pi * 60, 180 / np.pi * 60, 3600, 3600]
                )
            )
    ...
    residual_max = np.abs(residuals).max(axis=0) * 3600.0
    ...
    fullsol = np.array((gamma - 1.0, theta_rad, thetay_rad, deltax, deltay))
...
def plot_fit(coord, coord_obs, residuals, labels=None, sol=None):
    ...
    if labels is not None:
        for i in range(n):
            plt.text(
                coord_arr[i, 0] * 3600, coord_arr[i, 1] * 3600, labels[i], color="k"
            )
    qv = plt.quiver(
        coord_arr[:, 0] * 3600,
        coord_arr[:, 1] * 3600,
    ...
    plt.quiverkey(qv, 0.8, 0.9, 10, "10 arcsec", color="k")
    plt.ylim(plt.ylim()[::-1])
    ...
    if labels is not None:
        for i in range(n):
            plt.text(
                coord_obs_arr[i, 0] * 3600,
                coord_obs_arr[i, 1] * 3600,
                labels[i],
                color="k",
            )
    qv = plt.quiver(
        coord_obs_arr[:, 0] * 3600,
        coord_obs_arr[:, 1] * 3600,
        residuals[:, 0] * 3600,
        residuals[:, 1] * 3600,
    )
    plt.quiverkey(qv, 0.8, 0.9, 10, "10 arcsec", color="k")
...
def add_hystcomp(moves):
    ...
    delta = 30.0
    precomp1 = np.array([0.0, -delta]) * np.sign(moves[0, 1])
    ...
    precomp_rev = np.array([0.0, -delta]) * np.sign(moves[nmoves, 1])
    ...
    for i in np.arange(nmoves - 1) + nmoves:
        if np.sign(moves[i, 1]) != np.sign(moves[i + 1, 1]):
            precomp = np.array([[0.0, -delta], [0.0, delta]]) * np.sign(moves[i + 1, 1])
            precomp[0] += moves[i]
...
def compute_autocoll_moves(targets_coord, hystcomp=False, CEg=1.02928, Elg=1.00379):
    ...
    for i, c in enumerate(targets_coord):
        # frame move
        theta_EL_CE[i, 0] = c.lon.deg / 2.0 / Elg
        theta_EL_CE[i, 1] = -c.lat.deg / 2.0 / CEg
        print(
            "EL: {:.1f} arcsec ; CE: {:.1f} arcsec".format(
                theta_EL_CE[i, 0] * 3600, theta_EL_CE[i, 1] * 3600
            )
        )
...
class Guider2UV(object):
    ...
    def __init__(
        ...
        mask_rotation=0.0 * u.deg,
        FOVcenter_guider_coord=None,
        Field_gamma=1.0,  # platescale correction
        guider_wcs=None,
        filename=None,
    ):
        """
        ...
        if filename is None:
            ...
            self.FieldP = LocalScienceMaskProjector(
                Field_center, Field_rotation, Field_gamma
            )
...
""".format(
            ...
            self.GuiderP,
            self.mask_rotation,
            self.FOV_center_guider_pix[0],
            self.FOV_center_guider_pix[1],
        )
    ...
    def copy(self):
        return Guider2UV(
            guider_wcs=self.GuiderP.w.copy(),
    ...
    def set_FOV_center(
        ...
        self.FOV_center_guider_coord = coordinates.SkyCoord(
            FOV_center_guider_lon, FOV_center_guider_lat, frame=self.GuiderP.localframe
        )
    ...
    def UVstar_guider_coord(
        self, StarsGuider_pos, Stars_id=[0, 1, 2], UVStar_id=3, FourStarsGuider_pos=None
    ):
        ...
        UVstar_coord = register_UV_star_simple(
            this_pattern_coord, Stars_coord, Stars_id, UVStar_id
        )

        return coordinates.SkyCoord(
            UVstar_coord[0] * u.deg,
    ...
    def FieldLocal_to_guider(self, Fcoord_local, angle=False):
        ...
        Fcoord_local = coordinates.SkyCoord(
            local_lon, local_lat, frame=self.GuiderP.localframe
        )
        if angle:
            return Fcoord_local
        else:
    ...
    def guider_to_FieldLocal(self, Gcoord_local, angle=False):
        ...
        return coordinates.SkyCoord(
            F_local[0] * u.deg, F_local[1] * u.deg, frame=self.FieldP.localframe
        )
    ...
    def set_detector_mapping(self, mapping, offsets=[1.0, 1.0]):
        ...
        self.inv_map = lambda w, x, y: self.mask_det_map.inv_map(
            w, x - offsets[0], y - offsets[1]
        )
    ...
    def compute_autocoll_moves_slits(
        self, slits, slit_table, hystcomp=False, CEg=1.02928, Elg=1.00379
    ):
        ...
        for s in slits:
            slit_pos = np.array(
                [
                    st[st["Internal-count"] == s]["xmm"][0],
                    st[st["Internal-count"] == s]["ymm"][0],
                ]
            )
    ...
    def compute_autocoll_move_stars(
        self, stars, star_table, hystcomp=False, CEg=1.02928, Elg=1.00379
    ):
        ...
        for s in stars:
            ...
            star_coords.append(
                self.SienceMask2guider(star_pos_radec, world=True, angle=True)
            )
    ...
    def pattern_in_slit(self, starid, Slit_pos, world=False, FourStarsGuider_pos=None):
        ...
        pattern_origin_Field_coord_lon = (
            slit_local_coord.lon - this_pattern_coord[starid].lon
        )
        pattern_origin_Field_coord_lat = (
            slit_local_coord.lat - this_pattern_coord[starid].lat
        )

        pattern_Field_coord_lon = (
            pattern_origin_Field_coord_lon + this_pattern_coord.lon
        )
        pattern_Field_coord_lat = (
            pattern_origin_Field_coord_lat + this_pattern_coord.lat
        )
        pattern_Field_coord = coordinates.SkyCoord(
            pattern_Field_coord_lon,
            pattern_Field_coord_lat,
    ...
    def save(self, filename="Guider2UV.pkl"):
        ...
        data = {
            "Fcenter": self.FieldP.center,
            "Frot": self.FieldP.rotation,
            "Gwcs": self.GuiderP.w,
        }

        if self.FieldP.gamma != 1.0:
        ...
        try:
            data["FOVcenter"] = (
                self.FOV_center_guider_coord.lon,
                self.FOV_center_guider_coord.lat,
            )
    ...
    def restore(self, filename="Guider2UV.pkl"):
        ...
        try:
            # restore pattern
            self.pattern_coord = coordinates.SkyCoord(
                data["pattern"][0], data["pattern"][1], frame=self.GuiderP.localframe
            )
        except KeyError:
            pass
    ...
    def update_model(
        ...
        new_offset = [
            -theta_rad * G2UVcor.FOV_center_guider_coord.lat + deltax,
            theta_rad * G2UVcor.FOV_center_guider_coord.lon + deltay,
        ]

        newFOVcenter = coordinates.SkyCoord(
            G2UVcor.FOV_center_guider_coord.lon * gamma + new_offset[0],
        ...
        for i in range(n):
            new_coord.append(
                coordinates.SkyCoord(
                    new[i, 0] * u.deg, new[i, 1] * u.deg, frame=self.GuiderP.localframe
                )
            )
...
if __name__ == "__main__":
    ...
    new_F1_star_pattern_tab = Table.read(new_F1_star_pattern_filename)
    new_F1_star_pattern_tab.sort("xcentroid")
    new_F1_star_pattern_pos = np.array(
        [new_F1_star_pattern_tab["xcentroid"], new_F1_star_pattern_tab["ycentroid"]]
    ).T
    ...
    G2UV.set_FOV_center(
        new_F1_star_pattern_pos, star_UV_pos, slit_pos_ang, UVStar_id=3, world=True
    )
