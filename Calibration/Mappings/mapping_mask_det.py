...
if __name__ == '__main__':
    ...
    # path = "/Users/Vincent/Nextcloud/LAM/FIREBALL/all_diffuse_illumination/2023/16_cold_230612/F1_image000009.csv"
    # t=Table.read(path)
    # t["wavelength"] = t["line"]
    ...
    #                       savename=path.replace(".csv",".reg"), color = colors, ID=ID)
...
#%%
    ...
    for field in ["F3"]:
        path = "/Users/Vincent/Library/CloudStorage/GoogleDrive-vp2376@columbia.edu/.shortcut-targets-by-id/1ZgB7kY-wf7meXrq8v-1vIzor75aRdLDn/FIREBall-2/FB2_2023/instrument_alignment_focusing/XY_calibration/FireBallPipe/Calibration/Targets/2022/targets_%s.csv"%(field)
        # path="/Users/Vincent/Nextcloud/LAM/FIREBALL/all_diffuse_illumination/2023/16_cold_230612/230615/F3_image000018.csv"
        slits = Table.read(path)
        mapping = Mapping("/Users/Vincent/Library/CloudStorage/GoogleDrive-vp2376@columbia.edu/.shortcut-targets-by-id/1ZgB7kY-wf7meXrq8v-1vIzor75aRdLDn/FIREBall-2/FB2_2023/instrument_alignment_focusing/XY_calibration/FireBallPipe/Calibration/Mappings/2023/mask_to_det_mapping/mapping-mask-det-w-2023-0-%s_59.5.pkl"%(field))
        mapping = Mapping("/Users/Vincent/Library/CloudStorage/GoogleDrive-vp2376@columbia.edu/.shortcut-targets-by-id/1ZgB7kY-wf7meXrq8v-1vIzor75aRdLDn/FIREBall-2/FB2_2023/instrument_alignment_focusing/XY_calibration/FireBallPipe/Calibration/Mappings/2023/mask_to_det_mapping/mapping-mask-det-w-2023-0-QSO_10125.pkl")
        ...
        #                           savename=path.replace(".csv",".reg"), color = colors, ID=[ID]*3)
        # else:
        #     create_ds9_regions(xdetpix, ydetpix, form=['box']*3, radius=[10,32], save=True, 
        #                           savename=path.replace(".csv",".reg"), color = colors, ID=[ID]*3)
        if field in ["F1","F4"]:
            create_ds9_regions(xdetpix, ydetpix, form=['box']*3, radius=[10,20], save=True, 
                                  savename=path.replace(".csv","_no_OS.reg"), color = colors, ID=[ID]*3)
        else:
            create_ds9_regions(xdetpix, ydetpix, form=['box']*3, radius=[10,32], save=True, 
                                  savename=path.replace(".csv","_no_OS.reg"), color = colors, ID=[ID]*3)
