...
def load_pickle(pickle_file):
    ...
    ):  # needed for DSutils: package name is changed to DS9FireBall
...
class Mapping(object):
        ...
        if self.w is not None:
            if self.symmetric:
                ...
                for wi in self.w:
                    ...
                    if np.any(idx):
                        self.mapping.append(
                            PolyFit((xin[idx], yin[idx]), xy_out[idx], deg)
                        )
                        self.inv_mapping.append(
                            PolyFit((xout[idx], yout[idx]), xy_in[idx], deg)
                        )
                    else:
    ...
    def linear(self, center=(0.0, 0.0), w=None, inverse=False):
        """
        ...
        for m, w in zip(mapping, ww):
            ...
            coeffs = np.array(
                [[[cm[0], J[0, 0]], [J[0, 1], 0.0]], [[cm[1], J[1, 0]], [J[1, 1], 0.0]]]
            )
    ...
    def plot(
        ...
        for w in ww:
            ...
            ax.text(
                0.5,
                +0.02,
                "difference exagerated 10 times",
                color="b",
